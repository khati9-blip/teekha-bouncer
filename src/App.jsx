...
${aiMatchText.slice(0, 3000)}`;
  ...
  useEffect(() => {
    if (page === "matches") {
      fetchLiveScores();
      pollRef.current = setInterval(fetchLiveScores, 300000); // 5 minutes - reduced from 1 min to save quota
    }
  ...
  useEffect(() => {
    ...
    masterCheck();
    const masterInterval = setInterval(masterCheck, 180 * 1000); // 3 minutes - reduced from 1 min to save quota
  ...
  useEffect(() => {
    const t = setInterval(() => setSnatchWindowStatus(getSnatchWindowStatus()), 300000); // 5 minutes - reduced from 1 min to save quota
    return () => clearInterval(t);
  }, []);
  ...
  {navItems.map(n=>(
    <button key={n.id} onClick={()=>!n.disabled&&nav(n.id)}
      ...
      {page===n.id && <div style={{position:"absolute",top:0,left:"50%",transform:"translateX(-50%)",width:"60%",height:3,background:"linear-gradient(90deg, transparent 0%, #FFD700 50%, transparent 100%)",boxShadow:"0 0 8px #FFD700"}}/>}
        ...
        <div style={{maxWidth:page==="transfer"?1200:860,margin:"0 auto",padding:"20px 16px 90px"}}>
