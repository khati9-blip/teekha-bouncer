                  ...
                  <tbody>
                    {playingPlayers.map(p=>(
                      <tr key={p.id} style={{borderBottom:"1px solid #1E2D4533"}}>
                        <td style={{padding:"7px 6px",fontSize:13,fontWeight:600}}>{(()=>{const ts=getTradeStatus(team.id,p.id);const exP=ts?players.find(x=>x.id===ts.exchangedWith):null;return(<><div style={{display:"flex",alignItems:"center",gap:4}}>{ts&&<span style={{fontSize:11}}>{ts.type==="in"?"⬆️":"⬇️"}</span>}<span style={{color:ts?.type==="in"?"#2ECC71":ts?.type==="out"?"#FF3D5A":"#E2EAF4",textDecoration:ts?.type==="out"?"line-through":"none"}}>{p.name}</span> {p.tier&&<span style={{fontSize:9,fontWeight:800,letterSpacing:1,padding:"1px 5px",borderRadius:4,fontFamily:"Barlow Condensed,sans-serif",textTransform:"uppercase",background:p.tier==="platinum"?"#4A5E7833":p.tier==="gold"?"#F5A62322":p.tier==="silver"?"#94A3B822":"#CD7F3222",border:"1px solid "+(p.tier==="platinum"?"#4A5E7866":p.tier==="gold"?"#F5A62366":p.tier==="silver"?"#94A3B855":"#CD7F3255"),color:p.tier==="platinum"?"#B0BEC5":p.tier==="gold"?"#F5A623":p.tier==="silver"?"#94A3B8":"#CD7F32"}}>{p.tier==="platinum"?"PLAT":p.tier==="gold"?"GOLD":p.tier==="silver"?"SILV":"BRNZ"}</span>}{ts&&exP&&<span style={{fontSize:9,color:"#4A5E78",marginLeft:2}}>({ts.type==="in"?"for":"→"}{exP.name})</span>}</div><span style={{fontSize:10,color:"#4A5E78"}}>{p.iplTeam} • {p.role}</span></>);})()}</td>
          ...
          <div style={{position:"fixed",inset:0,background:"rgba(8,12,20,0.97)",zIndex:200,overflowY:"auto",padding:24,fontFamily:"Barlow Condensed,sans-serif"}}>
            <div style={{maxWidth:500,margin:"0 auto"}}>
              ...
              <div style={{background:"#0E1521",borderRadius:12,border:"1px solid #1E2D45",padding:20,marginBottom:16}}>
                ...
                {[
                  ["Transfer Window (Release)", "Sunday 11:59 PM → Monday 11:00 AM IST"],
                  ["Transfer Window (Trade)", "Monday 11:00 AM onwards (Admin triggered)"],
